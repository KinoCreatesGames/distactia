class Camera extends dn.Process {
  ...
  inline function get_left()
    return M.imax(M.floor(focus.levelX - pxWid * 0.5),
      clampToLevelBounds ? 0 : -Const.INFINITE);
  ...
  inline function get_top()
    return M.imax(M.floor(focus.levelY - pxHei * 0.5),
      clampToLevelBounds ? 0 : -Const.INFINITE);
  ...
  function apply() {
    ...
    if (clampToLevelBounds) {
      if (pxWid < level.cWid * Const.GRID) scroller.x = M.fclamp(scroller.x,
        pxWid - level.pxWid, 0);
      if (pxHei < level.cHei * Const.GRID) scroller.y = M.fclamp(scroller.y,
        pxHei - level.pxHei, 0);
    }
